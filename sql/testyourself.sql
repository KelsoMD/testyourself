-- --------------------------------------------------------
-- Хост:                         127.0.0.1
-- Версия сервера:               5.7.21-log - MySQL Community Server (GPL)
-- Операционная система:         Win64
-- HeidiSQL Версия:              9.5.0.5196
-- --------------------------------------------------------

/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET NAMES utf8 */;
/*!50503 SET NAMES utf8mb4 */;
/*!40014 SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0 */;
/*!40101 SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO' */;


-- Дамп структуры базы данных testyourself
DROP DATABASE IF EXISTS `testyourself`;
CREATE DATABASE IF NOT EXISTS `testyourself` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `testyourself`;

-- Дамп структуры для таблица testyourself.groups
DROP TABLE IF EXISTS `groups`;
CREATE TABLE IF NOT EXISTS `groups` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `mentor` int(10) unsigned NOT NULL,
  `theme` varchar(50) NOT NULL,
  `title` varchar(50) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `mentor` (`mentor`),
  KEY `theme` (`theme`),
  CONSTRAINT `FK1_group_mentor` FOREIGN KEY (`mentor`) REFERENCES `users` (`id`),
  CONSTRAINT `FK2_group_theme` FOREIGN KEY (`theme`) REFERENCES `themes` (`theme`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8;

-- Дамп данных таблицы testyourself.groups: ~6 rows (приблизительно)
/*!40000 ALTER TABLE `groups` DISABLE KEYS */;
REPLACE INTO `groups` (`id`, `mentor`, `theme`, `title`) VALUES
	(1, 1, 'geography', 'first'),
	(2, 2, 'geography', 'second'),
	(3, 2, 'starwars', 'warsstars'),
	(4, 2, 'geography', 'java'),
	(5, 2, 'java', 'Java tests'),
	(6, 2, 'geography', 'midnight');
/*!40000 ALTER TABLE `groups` ENABLE KEYS */;

-- Дамп структуры для таблица testyourself.group_members
DROP TABLE IF EXISTS `group_members`;
CREATE TABLE IF NOT EXISTS `group_members` (
  `member` int(10) unsigned DEFAULT NULL,
  `group_id` int(10) unsigned DEFAULT NULL,
  KEY `member` (`member`),
  KEY `group` (`group_id`),
  CONSTRAINT `FK1_member` FOREIGN KEY (`member`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK2_group_id` FOREIGN KEY (`group_id`) REFERENCES `groups` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Дамп данных таблицы testyourself.group_members: ~0 rows (приблизительно)
/*!40000 ALTER TABLE `group_members` DISABLE KEYS */;
/*!40000 ALTER TABLE `group_members` ENABLE KEYS */;

-- Дамп структуры для таблица testyourself.group_task
DROP TABLE IF EXISTS `group_task`;
CREATE TABLE IF NOT EXISTS `group_task` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `party` int(11) unsigned NOT NULL,
  `test` int(11) unsigned NOT NULL,
  `finish_to` date NOT NULL,
  PRIMARY KEY (`id`),
  KEY `group` (`party`),
  KEY `test` (`test`),
  CONSTRAINT `FK1_task_group` FOREIGN KEY (`party`) REFERENCES `groups` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK2_task_test` FOREIGN KEY (`test`) REFERENCES `tests` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8;

-- Дамп данных таблицы testyourself.group_task: ~2 rows (приблизительно)
/*!40000 ALTER TABLE `group_task` DISABLE KEYS */;
REPLACE INTO `group_task` (`id`, `party`, `test`, `finish_to`) VALUES
	(1, 1, 1, '2018-04-29'),
	(2, 2, 1, '2018-05-23');
/*!40000 ALTER TABLE `group_task` ENABLE KEYS */;

-- Дамп структуры для таблица testyourself.invite
DROP TABLE IF EXISTS `invite`;
CREATE TABLE IF NOT EXISTS `invite` (
  `group_id` int(10) unsigned DEFAULT NULL,
  `member` int(10) unsigned DEFAULT NULL,
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  PRIMARY KEY (`id`),
  KEY `group_id` (`group_id`),
  KEY `member` (`member`),
  CONSTRAINT `FK1_group_invite` FOREIGN KEY (`group_id`) REFERENCES `groups` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK2_member_invite` FOREIGN KEY (`member`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Дамп данных таблицы testyourself.invite: ~0 rows (приблизительно)
/*!40000 ALTER TABLE `invite` DISABLE KEYS */;
/*!40000 ALTER TABLE `invite` ENABLE KEYS */;

-- Дамп структуры для таблица testyourself.questions
DROP TABLE IF EXISTS `questions`;
CREATE TABLE IF NOT EXISTS `questions` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `theme` varchar(50) DEFAULT NULL,
  `level` enum('1','2','3') DEFAULT NULL,
  `text` varchar(255) DEFAULT NULL,
  `image` varchar(255) DEFAULT NULL,
  `answer_correct` varchar(255) DEFAULT NULL,
  `answer_1` varchar(255) DEFAULT NULL,
  `answer_2` varchar(255) DEFAULT NULL,
  `answer_3` varchar(255) DEFAULT NULL,
  `author` int(10) unsigned DEFAULT NULL,
  `aprooved` tinyint(3) unsigned DEFAULT '0',
  PRIMARY KEY (`id`),
  KEY `theme` (`theme`),
  KEY `author` (`author`),
  CONSTRAINT `FK1_theme` FOREIGN KEY (`theme`) REFERENCES `themes` (`theme`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK2_author` FOREIGN KEY (`author`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=24 DEFAULT CHARSET=utf8;

-- Дамп данных таблицы testyourself.questions: ~21 rows (приблизительно)
/*!40000 ALTER TABLE `questions` DISABLE KEYS */;
REPLACE INTO `questions` (`id`, `theme`, `level`, `text`, `image`, `answer_correct`, `answer_1`, `answer_2`, `answer_3`, `author`, `aprooved`) VALUES
	(1, 'geography', '1', 'Столица Республики Беларусь', NULL, 'Минск', 'Москва', 'Могилев', 'Пинск', 1, 1),
	(2, 'java', '1', 'question text', NULL, 'correct', '1', '2', '3', 1, 1),
	(3, 'math', '1', 'question text', NULL, 'correct', '1', '2', '3', 1, 1),
	(5, 'geography', '1', 'Столица Нидерландов', NULL, 'Амсердам', 'Антховен', 'Гаага', 'Брюге', 1, 1),
	(6, 'geography', '1', 'Самая высока гора в мире', NULL, 'Эверест', 'Арарат', 'Анды', 'Не знаю больше гор', 1, 1),
	(7, 'geography', '1', 'Количество материков на планете', NULL, '6', '5', '7', '4', 1, 1),
	(9, 'geography', '1', 'Назовите город на фотографии', 'http://ipic.su/img/img7/fs/london-kerc-VK.1523363480.jpg', 'Лондон', 'Париж', 'Нью-Йорк', 'Москва', 1, 1),
	(10, 'geography', '1', 'Назовите страну', 'http://ipic.su/img/img7/fs/1200px-Flag_of_Australia_(converted).1523363571.png', 'Австралия', 'Великобритания', 'Канада', 'Новая Зеландия', 1, 1),
	(11, 'geography', '1', 'Назовите страну', 'http://ipic.su/img/img7/fs/53b3b29c98281b66730a0f2a77082c0a.1523363641.jpg', 'Япония', 'Китай', 'Казахстан', 'Вьетнам', 1, 1),
	(12, 'geography', '1', 'Назовите город-государство', NULL, 'Ватикан', 'Калининград', 'Барселона', 'Ницца', 1, 1),
	(13, 'geography', '1', 'Континент на котором находится Республика Беларусь', NULL, 'Евразия', 'Европа', 'Азия', 'Беларусостан', 1, 1),
	(14, 'geography', '1', 'Назовите страну', 'http://ipic.su/img/img7/fs/Beznazvaniya(1).1523363922.jpg', 'Австралия', 'Китай', 'Индия', 'Япония', 1, 1),
	(15, 'starwars', '1', 'Количество эпизодов приквелов', NULL, '3', '2', '4', '6', 1, 1),
	(16, 'starwars', '1', 'Назовите персонажа', 'http://ipic.su/img/img7/fs/Esb14.1523364121.jpg', 'Чубакка', 'Эвок', 'Йода', 'Принцесса Лея', 1, 1),
	(17, 'starwars', '1', 'Название космической станции Империи', NULL, 'Звезда смерти', 'Смерть звезд', 'Звездный разрушитель', 'Сокол тысячелетия', 1, 1),
	(18, 'starwars', '1', 'Как называют детей начавших обучение пути Джедая', NULL, 'Юнленг', 'Падаван', 'Маленький джедай', 'Нуб', 1, 1),
	(19, 'starwars', '1', 'Фамилия главных действующи персонажей саги', NULL, 'Скайвокер', 'Бэггинс', 'Соло', 'Холмс', 1, 1),
	(20, 'starwars', '1', 'Как называют этих солдат Империи', 'http://ipic.su/img/img7/fs/SS2124_01.1523364424.jpg', 'Штурмовик', 'Десантник', 'Морпех', 'Имперский гвардеец', 1, 1),
	(21, 'geography', '1', 'bla', '', '1', '2', '3', '4', 2, 0),
	(22, 'math', '2', '1+1', '', '1', '2', '3', '4', 2, 0),
	(23, 'geography', '1', 'test question', '', '1', '2', '3', '4', 5, 0);
/*!40000 ALTER TABLE `questions` ENABLE KEYS */;

-- Дамп структуры для таблица testyourself.result
DROP TABLE IF EXISTS `result`;
CREATE TABLE IF NOT EXISTS `result` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `test` int(10) unsigned DEFAULT NULL,
  `mark` int(11) unsigned DEFAULT NULL,
  `passed` tinyint(3) unsigned DEFAULT NULL,
  `user` int(10) unsigned DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `test` (`test`),
  KEY `user` (`user`),
  CONSTRAINT `FK1_result_test` FOREIGN KEY (`test`) REFERENCES `tests` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK2_result_user` FOREIGN KEY (`user`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8;

-- Дамп данных таблицы testyourself.result: ~6 rows (приблизительно)
/*!40000 ALTER TABLE `result` DISABLE KEYS */;
REPLACE INTO `result` (`id`, `test`, `mark`, `passed`, `user`) VALUES
	(1, 4, 83, 1, 2),
	(2, 4, 100, 1, 2),
	(3, 6, 100, 1, 2),
	(4, 1, 80, 1, 2),
	(5, 4, 33, 0, 2),
	(6, 2, 0, 0, 2);
/*!40000 ALTER TABLE `result` ENABLE KEYS */;

-- Дамп структуры для таблица testyourself.task-result
DROP TABLE IF EXISTS `task-result`;
CREATE TABLE IF NOT EXISTS `task-result` (
  `member` int(10) unsigned DEFAULT NULL,
  `task` int(10) unsigned DEFAULT NULL,
  `date` date DEFAULT NULL,
  `mark` date DEFAULT NULL,
  `passed` date DEFAULT NULL,
  `id` int(11) NOT NULL AUTO_INCREMENT,
  PRIMARY KEY (`id`),
  KEY `member` (`member`),
  KEY `task` (`task`),
  CONSTRAINT `FK1_task_result_member` FOREIGN KEY (`member`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK2_task_task` FOREIGN KEY (`task`) REFERENCES `group_task` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Дамп данных таблицы testyourself.task-result: ~0 rows (приблизительно)
/*!40000 ALTER TABLE `task-result` DISABLE KEYS */;
/*!40000 ALTER TABLE `task-result` ENABLE KEYS */;

-- Дамп структуры для таблица testyourself.tests
DROP TABLE IF EXISTS `tests`;
CREATE TABLE IF NOT EXISTS `tests` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `theme` varchar(50) DEFAULT NULL,
  `level` enum('1','2','3') DEFAULT NULL,
  `author` int(10) unsigned DEFAULT NULL,
  `aprooved` tinyint(3) unsigned DEFAULT '0',
  PRIMARY KEY (`id`),
  KEY `theme` (`theme`),
  KEY `author` (`author`),
  CONSTRAINT `FK1_theme_test` FOREIGN KEY (`theme`) REFERENCES `themes` (`theme`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK2_author_test` FOREIGN KEY (`author`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=utf8;

-- Дамп данных таблицы testyourself.tests: ~8 rows (приблизительно)
/*!40000 ALTER TABLE `tests` DISABLE KEYS */;
REPLACE INTO `tests` (`id`, `theme`, `level`, `author`, `aprooved`) VALUES
	(1, 'geography', '1', 1, 1),
	(2, 'java', '1', 1, 1),
	(3, 'math', '1', 1, 1),
	(4, 'starwars', '1', 1, 1),
	(6, 'geography', '1', 2, 0),
	(7, 'geography', '1', 2, 0),
	(8, 'geography', '1', 2, 0),
	(9, 'geography', '1', 2, 0);
/*!40000 ALTER TABLE `tests` ENABLE KEYS */;

-- Дамп структуры для таблица testyourself.test_questions
DROP TABLE IF EXISTS `test_questions`;
CREATE TABLE IF NOT EXISTS `test_questions` (
  `test` int(10) unsigned DEFAULT NULL,
  `question` int(10) unsigned DEFAULT NULL,
  KEY `test` (`test`),
  KEY `question` (`question`),
  CONSTRAINT `FK1 _test_1` FOREIGN KEY (`test`) REFERENCES `tests` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK2_question_1` FOREIGN KEY (`question`) REFERENCES `questions` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Дамп данных таблицы testyourself.test_questions: ~26 rows (приблизительно)
/*!40000 ALTER TABLE `test_questions` DISABLE KEYS */;
REPLACE INTO `test_questions` (`test`, `question`) VALUES
	(1, 1),
	(2, 2),
	(3, 3),
	(1, 5),
	(1, 6),
	(1, 7),
	(1, 9),
	(1, 10),
	(1, 11),
	(1, 12),
	(1, 13),
	(1, 14),
	(4, 15),
	(4, 16),
	(4, 17),
	(4, 18),
	(4, 19),
	(4, 20),
	(6, 10),
	(6, 7),
	(7, 11),
	(8, 12),
	(8, 7),
	(8, 14),
	(9, 13),
	(9, 14);
/*!40000 ALTER TABLE `test_questions` ENABLE KEYS */;

-- Дамп структуры для таблица testyourself.themes
DROP TABLE IF EXISTS `themes`;
CREATE TABLE IF NOT EXISTS `themes` (
  `theme` varchar(50) NOT NULL DEFAULT 'undefined',
  `aprooved` tinyint(3) unsigned DEFAULT '0',
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  PRIMARY KEY (`id`),
  UNIQUE KEY `theme` (`theme`)
) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=utf8;

-- Дамп данных таблицы testyourself.themes: ~8 rows (приблизительно)
/*!40000 ALTER TABLE `themes` DISABLE KEYS */;
REPLACE INTO `themes` (`theme`, `aprooved`, `id`) VALUES
	('geography', 1, 1),
	('java', 1, 2),
	('math', 1, 3),
	('starwars', 1, 4),
	('theme_new', 0, 5),
	('bla', 0, 6),
	('bloblo', 0, 7),
	('blobloblo', 0, 8);
/*!40000 ALTER TABLE `themes` ENABLE KEYS */;

-- Дамп структуры для таблица testyourself.users
DROP TABLE IF EXISTS `users`;
CREATE TABLE IF NOT EXISTS `users` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `login` varchar(12) DEFAULT NULL,
  `pass` varchar(12) DEFAULT NULL,
  `name` varchar(50) DEFAULT NULL,
  `surname` varchar(50) DEFAULT NULL,
  `e_mail` varchar(50) DEFAULT NULL,
  `avatar` blob,
  `role` enum('ROLE_USER','ROLE_MODERATOR','ROLE_MENTOR','ROLE_ADMIN') DEFAULT 'ROLE_USER',
  `theme` varchar(50) DEFAULT NULL,
  `enabled` tinyint(4) NOT NULL DEFAULT '1',
  PRIMARY KEY (`id`),
  UNIQUE KEY `login` (`login`),
  KEY `theme` (`theme`),
  CONSTRAINT `FK1_mentor_theme` FOREIGN KEY (`theme`) REFERENCES `themes` (`theme`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=utf8;

-- Дамп данных таблицы testyourself.users: ~5 rows (приблизительно)
/*!40000 ALTER TABLE `users` DISABLE KEYS */;
REPLACE INTO `users` (`id`, `login`, `pass`, `name`, `surname`, `e_mail`, `avatar`, `role`, `theme`, `enabled`) VALUES
	(1, 'admin', 'admin', 'admin', 'admin', 'admin', _binary , 'ROLE_ADMIN', NULL, 1),
	(2, 'test', 'test', 'test', 'test', 'test', NULL, 'ROLE_USER', NULL, 1),
	(5, 'user', 'user', 'Alex', 'Nesterenok', 'aleksejnesterenok@gmail.com', NULL, 'ROLE_USER', NULL, 1),
	(6, 'mentor', 'mentor', 'Alex', 'Nesterenok', 'aleksejnesterenok@gmail.com', NULL, 'ROLE_MENTOR', NULL, 1),
	(7, 'moderator', 'moderator', 'Alex', 'Nesterenok', 'aleksejnesterenok@gmail.com', NULL, 'ROLE_MODERATOR', NULL, 1);
/*!40000 ALTER TABLE `users` ENABLE KEYS */;

-- Дамп структуры для таблица testyourself.user_groups
DROP TABLE IF EXISTS `user_groups`;
CREATE TABLE IF NOT EXISTS `user_groups` (
  `user_id` int(10) unsigned DEFAULT NULL,
  `group_id` int(10) unsigned DEFAULT NULL,
  KEY `user` (`user_id`),
  KEY `group` (`group_id`),
  CONSTRAINT `FK1_user_groups` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK2_user_groupsoff` FOREIGN KEY (`group_id`) REFERENCES `groups` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Дамп данных таблицы testyourself.user_groups: ~0 rows (приблизительно)
/*!40000 ALTER TABLE `user_groups` DISABLE KEYS */;
/*!40000 ALTER TABLE `user_groups` ENABLE KEYS */;

/*!40101 SET SQL_MODE=IFNULL(@OLD_SQL_MODE, '') */;
/*!40014 SET FOREIGN_KEY_CHECKS=IF(@OLD_FOREIGN_KEY_CHECKS IS NULL, 1, @OLD_FOREIGN_KEY_CHECKS) */;
/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
