...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.csrf().disable();
        http.logout().logoutSuccessUrl("/guest")
        ...
        http.authorizeRequests()
                ...
                .antMatchers("/guest/**").anonymous().and();
        http.formLogin()
                .loginPage("/login")
                ...
                .passwordParameter("password_login").isCustomLoginPage();
        http.httpBasic();
    }
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
        auth.jdbcAuthentication().dataSource(this.dataSource()).usersByUsernameQuery(
                "select login, pass, enabled from users where login=?")
    ...
    public DriverManagerDataSource dataSource() {
        ...
        driverManagerDataSource.setUrl("jdbc:mysql://localhost/testyourself?useTimezone=true&serverTimezone=UTC&useSSL=false");
